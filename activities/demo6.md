...
* **Create a new Topology with a Fortigate resource connected to L2 switch and using LAG feature for port connectivity (Automation scripts)**
    ...
    * Click (open in a new window) on "\[demo#6\] Topology 1" below in this activity page and go to the "Topology" page; click "Edit" to open the Topology for editing; click on "VLAN" cloud and check "ID" field in the left side of the page; you can define a custom VLAN ID or you can let Velocity choose a VLAN ID which is not used
    * In the Topology page please notice that connection type is LAG; this is the indicator that the User wnats to build LAG Ports dynamicaly by using Automation scripts 
    *
